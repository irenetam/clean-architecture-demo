using AutoMapper;
using Blazored.LocalStorage;
using TicketManagement.App.Contracts;
using TicketManagement.App.Services.Base;

namespace TicketManagement.App.Services
{
    public class EventDataService : BaseDataService, IEventDataService
    {
        private readonly IMapper _mapper;
        public EventDataService(IClient client, ILocalStorageService localStorage, IMapper mapper) : base(client, localStorage)
        {
            _mapper = mapper;
        }

        public async Task<List<ViewModels.EventListViewModel>> GetAllEvents()
        {
            var events = await _client.GetAllEventsAsync();
            var mappedEvents = _mapper.Map<ICollection<ViewModels.EventListViewModel>>(events);
            return mappedEvents.ToList();
        }

        public async Task<ViewModels.EventDetailViewModel> GetEventById(Guid id)
        {
            var selectedEvent = await _client.GetEventByIdAsync(id);
            var mappedEvent = _mapper.Map<ViewModels.EventDetailViewModel>(selectedEvent);
            return mappedEvent;
        }

        public async Task<ApiResponse<Guid>> CreateEvent(ViewModels.EventDetailViewModel eventDetailViewModel)
        {
            try
            {
                CreateEventCommand createEventCommand = _mapper.Map<CreateEventCommand>(eventDetailViewModel);
                var newId = await _client.AddEventAsync(createEventCommand);
                return new ApiResponse<Guid>() { Data = newId, Success = true };
            }
            catch (ApiException ex)
            {
                return ConvertApiExceptions<Guid>(ex);
            }
        }

        public async Task<ApiResponse<Guid>> UpdateEvent(ViewModels.EventDetailViewModel eventDetailViewModel)
        {
            try
            {
                UpdateEventCommand updateEventCommand = _mapper.Map<UpdateEventCommand>(eventDetailViewModel);
                await _client.UpdateEventAsync(updateEventCommand);
                return new ApiResponse<Guid>() { Success = true };
            }
            catch (ApiException ex)
            {
                return ConvertApiExceptions<Guid>(ex);
            }
        }

        public async Task<ApiResponse<Guid>> DeleteEvent(Guid id)
        {
            try
            {
                await _client.DeleteEventAsync(id);
                return new ApiResponse<Guid>() { Success = true };
            }
            catch (ApiException ex)
            {
                return ConvertApiExceptions<Guid>(ex);
            }
        }
    }
}
